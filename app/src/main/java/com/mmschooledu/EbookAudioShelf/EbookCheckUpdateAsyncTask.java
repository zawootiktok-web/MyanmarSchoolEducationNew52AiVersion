...
public class EbookCheckUpdateAsyncTask {
	...
	public EbookCheckUpdateAsyncTask(Activity activity, String json) {
        this.json = json;
    ...
    private void letUpdate(final String title, final String message, final boolean force){
		...
		View parent_view = LayoutInflater.from(activity).inflate(R.layout.ebook_two_button_dia, null);
		...
		iv.setImageResource(R.drawable.ebook_ic_update);
		...
		if (!force){
			cancel.setOnClickListener(new View.OnClickListener(){
					...
					{
						downloadFile(download_link, activity.getString(R.string.ebook_ebook_app_name) + "_v" + Integer.toString(versionCode) + ".apk");
    ...
    private void openFile(String apk){
		...
		if (Build.VERSION.SDK_INT >= 24) {
			fileUri = EbookFileProvider.getUriForFile(activity, activity. getPackageName() + ".provider",
												 f);
		}
